...
class ContourPlot(Plot):
    ...
    def __init__(self, **kwargs):
        ...
        self.bind(data=self.ask_draw,
                  xrange=self.ask_draw,
...
class BarPlot(Plot):
    ...
    def draw(self, *args):
        ...
        if len(points) * 6 > 65535:
            Logger.error("BarPlot: cannot support more than 10922 points. "
...
class ScatterPlot(Plot):
    ...
    def create_drawings(self):
        ...
        self._points_context = RenderContext(use_parent_modelview=True,
                                             use_parent_projection=True)
...
if __name__ == '__main__':
    ...
    class TestApp(App):
        ...
        def build(self):
            ...
            plot.points = [(x, .1 + randrange(10) / 10.)
                           for x in range(-50, 1)]
        ...
        def make_contour_data(self, ts=0):
            ...
            for ii, t in enumerate(time):
                for jj, x in enumerate(position):
                    data[ii,
                         jj] = sin(k * x +
                                   omega * t) + sin(-k * x + omega * t) / ts
            return (0, max(position)), (0, max(time)), data

        def update_points(self, *args):
            self.plot.points = [(x / 10., cos(Clock.get_time() + x / 50.))
